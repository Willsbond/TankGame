...
public class tankTwoController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (TurnManager.turn == true)
        {
            ...
            //on turret move key, adjust turret and fire position
            if (Input.GetButtonDown("TurretDown"))
            {
                ...
                //check where turret is and adjust fire position
                if (turretPos == 1)
                {
            ...
            if (Input.GetButtonDown("TurretUp"))
            {
                ...
                //check where turret is and move fire position
                if (turretPos == 1)
                {
                    this.gameObject.GetComponent<SpriteRenderer>().sprite = turretPosSprite1;
                }
    ...
    // if hit by missile and its not your turn take damage
    void OnTriggerEnter2D(Collider2D collision)
    {

        if (collision.tag == "Missile" && TurnManager.turn == false)
        {
            health = health - 10;
